...
function normalizeWorkedExample(example, index) { return { id: String(example?.id || `example-${index + 1}`), url: String(example?.url || ""), storagePath: String(example?.storagePath || ""), alt: String(example?.alt || "") }; }
function normalizeLesson(lesson) { const { speakingActivityId, ...rest } = lesson; return { ...rest, speakingPracticeId: String(lesson.speakingPracticeId || LEGACY_SPEAKING_REFERENCES[speakingActivityId] || ""), workedExamples: (Array.isArray(lesson.workedExamples) ? lesson.workedExamples : []).map(normalizeWorkedExample).filter(example => example.url) }; }
function normalizeUnit(unit) { return { ...unit, lessons: (unit.lessons || []).map(normalizeLesson) }; }
function cloudUnit(unit) { return { ...unit, lessons: unit.lessons.map(lesson => { const next = { ...lesson }; if (!next.workedExamples.length) delete next.workedExamples; return next; }) }; }
...
export async function saveUnit(unit, { requireCloud = false } = {}) { const clean = normalizeUnit(clone(unit)); const units = localUnits().map(normalizeUnit); const i = units.findIndex(x => x.id === clean.id); if (i < 0) units.push(clean); else units[i] = clean; const saveCloud = async () => { const s = await getFirebaseServices(); if (!s.auth.currentUser) throw new Error("Sign in with an authorised teacher account before saving Worked Examples."); await s.firestoreSdk.setDoc(s.firestoreSdk.doc(s.db, COLLECTION, clean.id), { ...cloudUnit(clean), updatedAt: s.firestoreSdk.serverTimestamp(), updatedBy: s.auth.currentUser.uid }); }; if (requireCloud) { await saveCloud(); cache(units); return clean; } cache(units); try { const s = await getFirebaseServices(); if (s.auth.currentUser) await saveCloud(); } catch (e) { console.info("[Units] Saved locally; cloud save unavailable.", e); } return clean; }
