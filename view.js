...
if (!unit) {
  ...
  app.innerHTML = `<a class="back-link" href="./">← All units</a><section class="unit-header"><div><p class="eyebrow">${yearLabel(unit.yearLevel)}</p><h1>${esc(unit.englishTitle)}</h1><p class="unit-chinese" lang="zh-Hans">${esc(unit.chineseTitle)}</p></div><div><strong>${unit.lessons.length} Lessons</strong><a class="button secondary" href="edit.html?unit=${encodeURIComponent(unit.id)}">Edit unit</a></div></section><section class="lesson-list">${unit.lessons.map((lesson, index) => {
    ...
    return `<article class="lesson" data-lesson-id="${esc(lesson.id)}"><div class="lesson-layout"><div class="lesson-content"><p class="eyebrow">Lesson ${index + 1}</p><h2>${esc(lesson.title || `Lesson ${index + 1}`)}</h2>${lesson.learningIntention ? `<div><h3>Learning intention</h3><p>${esc(lesson.learningIntention)}</p></div>` : ""}<div class="resource-grid"><div><h3>Vocabulary</h3>${vocabulary ? `<a href="${vocabularyUrl(vocabulary.id)}">${esc(vocabulary.title)} · ${vocabulary.itemCount} items →</a>` : `<p class="muted">Not linked</p>`}</div><div><h3>Speaking</h3>${speakingMarkup}</div></div>${lesson.notes ? `<div><h3>Activities / Notes</h3><p class="preline">${esc(lesson.notes)}</p></div>` : ""}${lesson.resources ? `<div><h3>Resources</h3><p class="links">${resources(lesson.resources)}</p></div>` : ""}</div>${workedExamples(lesson)}</div></article>`;
  }).join("")}</section>`;
